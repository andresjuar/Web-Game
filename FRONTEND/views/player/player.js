    ...
    `,
      ...
      function pickAvatar(emoji) {
        ...
        document.getElementById("avatar-display").childNodes[0].textContent =
          emoji;
  ...
  if (!code)  { alert("No room code found."); return; }
  ...
  ws.send("JOIN_ROOM", { code, name, victoryQuote, avatar: selectedAvatar }); 
      ...
      ws.on("LIAR_YOUR_TURN", ({ question, role, instruction }) => {
        ...
        badge.textContent = isSubject
          ...
          : "🎭 You're a Liar!";
        badge.className =
          "liar-role-badge " + (isSubject ? "is-subject" : "is-liar");
        ...
        document.getElementById("liar-write-instruction").textContent =
          instruction;
      ...
      ws.on(
        ...
        ({ questionIndex, totalQuestions, question, options, timeLimit }) => {
          ...
          document.getElementById("player-question-text").textContent =
            question;
